...
| `azurator export` | Create a new plaintext or SOPS-encrypted dotenv file and optional key map. |
...
Human-readable output is concise by default. Add `-v` before the command for
...
`export --key-map-out azurator.keys.json` creates the same reusable format from
the assignments selected for a new export. It is available with interactive,
`--select`, or `--all` selection and cannot be combined with `--key-map`.
...
Export requires exactly one new dotenv destination:

```bash
azurator export --out keys.env
azurator export --sops-out keys.enc.env
azurator export --sops-out keys.enc.env --key-map-out azurator.keys.json
...
See [Export and Refresh Keys](../../guides/export-keys/) for selection and file
guarantees.
