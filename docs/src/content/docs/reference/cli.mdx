...
selector-to-resource-and-slot mappings. Use that artifact with
`export --key-map azurator.keys.json`. A key map contains no values or local
source path, but its Azure resource IDs and selector names remain infrastructure
metadata.
